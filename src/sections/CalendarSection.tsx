...
export default function CalendarSection({ selectedDate, onSelectDate, onAgendar }: Props) {
  const today = dayjs();
      ...
      >
        <Container>
          <Grid container spacing={{ xs: 3, md: 4 }} alignItems="stretch">
            <Grid size={{ xs: 12, md: 6 }}>
              <Box sx={{ pr: { md: 3 } }}>
                ...
                <Stack direction="row" spacing={1} sx={{ mt: 2, flexWrap: "wrap", rowGap: 1 }}>
                  <Chip
                    icon={<ContentCutRoundedIcon sx={{ color: "var(--gold)" }} />}
                    label="Navalha aquecida"
                    variant="outlined"
                    sx={{ color: "var(--gold)", borderColor: "var(--gold)" }}
                  />
                  <Chip
                    icon={<LocalCafeRoundedIcon sx={{ color: "var(--gold)" }} />}
                    label="Espresso cortesia"
                    variant="outlined"
                    sx={{ color: "var(--gold)", borderColor: "var(--gold)" }}
                  />
                  <Chip
                    icon={<WatchLaterRoundedIcon sx={{ color: "var(--gold)" }} />}
                    label="Pontualidade"
                    variant="outlined"
                    sx={{ color: "var(--gold)", borderColor: "var(--gold)" }}
                  />
                ...
                <Stack spacing={1.2} sx={{ mt: 2.5 }}>
                  <Stack direction="row" spacing={1.2} alignItems="center">
                    ...
                    <Typography variant="body2" sx={{ opacity: 0.95 }}>
                      Rua dos Cavalheiros, 123 — Centro
                    </Typography>
                  </Stack>
                  <Stack direction="row" spacing={1.2} alignItems="center">
            ...
            <Grid size={{ xs: 12, md: 6 }}>
              <Card className="paper-card card-raise" sx={{ height: "100%" }} aria-label="Seletor de data para agendamento">
